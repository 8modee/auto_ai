...
def gather_status():
        ...
        if s.get("kill_window_days"):
            ...
            if created:
                try:
                    ...
                    kill_progress = min(100, int(age_days / s["kill_window_days"] * 100))
    ...
    for a in agents:
        ...
        error_rate = round((a.get("tasks_failed", 0) or 0) / total_tasks * 100, 1) if total_tasks else 0.0
        agent_rows.append({
            **a,
            "stale": stale,
            "hb_age": hb_age,
    ...
    return {
        ...
        "halted": halted,
        "overview": {
            "revenue_total": rev_all["t"] if rev_all else 0,
...
.card{background:var(--panel);border:1px solid var(--line);border-top:3px solid var(--accent);padding:2.5rem;width:min(400px,90vw);border-radius:8px;box-shadow:0 20px 60px rgba(0,0,0,.5)}
...
radial-gradient(ellipse 60% 45% at 50% 110%,rgba(188,140,255,.04),transparent),
...
/* ── KPI CARDS ── */
.kpi{background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:.9rem 1rem;position:relative;overflow:hidden;transition:transform .2s,border-color .2s}
...
.badge{font-family:var(--mono);font-size:.62rem;padding:.18rem .5rem;border-radius:20px;letter-spacing:.08em;text-transform:uppercase;white-space:nowrap}
...
/* ── APPROVALS ── */
.approval{border:1px solid var(--line);border-left:3px solid var(--amber);border-radius:8px;padding:.8rem .9rem;margin-bottom:.7rem;background:var(--bg2);transition:border-color .2s}
...
.modal{background:var(--panel);border:1px solid var(--red);border-top:4px solid var(--red);border-radius:10px;padding:1.8rem;width:min(440px,90vw)}
.modal h3{color:var(--red);font-size:1.2rem;margin-bottom:.8rem}
.modal p{color:var(--dim);font-size:.9rem;margin-bottom:1.2rem}
...
<div class="strip">
  <div class="brand"><span class="sigil"></span> THE INSTITUTION</div>
  <div class="stat"><b class="mono" id="s-runway">—</b><span>runway days</span></div>
  <div class="stat"><b class="mono green" id="s-rev">—</b><span>revenue all-time</span></div>
  <div class="stat"><b class="mono" id="s-agents">—</b><span>agents</span></div>
  <div class="stat"><b class="mono" id="s-compliance">—</b><span>compliance</span></div>
  <div class="spacer"></div>
  <div class="live"><span class="dot"></span> LIVE <span id="clock"></span></div>
  <span id="countdown">60s</span>
  <a class="btn sm" href="/manual">Stewardship Manual</a>
  <button class="btn sm red" onclick="openStop()">■ STOP ALL</button>
</div>

{% if halted %}
<div class="halt">
  <b>■ INSTITUTION HALTED</b>
  <span class="dim">Emergency stop is active. No autonomous work is running.</span>
  <button class="btn green sm" onclick="resume()">Resume the Institution</button>
</div>
{% endif %}
...
<div class="wrap">
  ...
  <div class="grid g-overview">
    <div class="kpi reveal"><div class="glow" style="background:var(--green)"></div>
      <div class="label">Runway</div>
      <div class="value" id="k-runway">—</div>
      <div class="sub" id="k-runway-trend">—</div>
    </div>
    <div class="kpi reveal"><div class="glow" style="background:var(--green)"></div>
      <div class="label">Revenue · all time</div>
      <div class="value green" id="k-rev-all">—</div>
      <div class="sub" id="k-rev-month">this month: —</div>
    </div>
    <div class="kpi reveal"><div class="glow" style="background:var(--blue)"></div>
      <div class="label">Agent workforce</div>
      <div class="value" id="k-agents">—</div>
      <div class="sub" id="k-services">—</div>
    </div>
    <div class="kpi reveal"><div class="glow" style="background:var(--amber)"></div>
      <div class="label">System health</div>
      <div class="value" id="k-health">—</div>
      <div class="sub" id="k-health-sub">—</div>
    </div>
    <div class="kpi reveal"><div class="glow" style="background:var(--purple)"></div>
      <div class="label">Constitutional compliance</div>
      <div class="value purple" id="k-compliance">—</div>
      <div class="sub" id="k-compliance-sub">—</div>
    </div>
    <div class="kpi reveal"><div class="glow" style="background:var(--cyan)"></div>
      <div class="label">Founder state</div>
      <div class="value" id="k-founder">—</div>
      <div class="sub" id="k-founder-sub">—</div>
    </div>
  </div>

  <!-- ═══ MAIN GRID: ONE ACTION + CHECK-IN ═══ -->
  <div class="grid g-main">
    <div class="action-card reveal" id="action-card">
      <div class="kicker">Today's single action</div>
      <div class="text" id="action-text">—</div>
      <div class="meta" id="action-meta"></div>
    </div>

    <div class="panel p-blue reveal">
      <header><span class="kicker">Human interface</span><h2>Daily check-in</h2></header>
      <div class="body">
        <form class="checkin" onsubmit="return sendCheckin(event)">
          <div class="ck-row"><label>Energy</label>
            <div class="seg">{% for i in [1,2,3,4,5] %}<input type="radio" name="energy" id="e{{i}}" value="{{i}}" {{ 'checked' if (overview.checkin and overview.checkin.energy==i) or (not overview.checkin and i==3) }}><label for="e{{i}}">{{i}}</label>{% endfor %}</div>
          </div>
          <div class="ck-row"><label>Pain</label>
            <div class="seg pain">{% for i in [1,2,3,4,5] %}<input type="radio" name="pain" id="p{{i}}" value="{{i}}" {{ 'checked' if (overview.checkin and overview.checkin.pain==i) or (not overview.checkin and i==3) }}><label for="p{{i}}">{{i}}</label>{% endfor %}</div>
          </div>
          <div class="ck-row"><label>Fear</label>
            <div class="seg fear">{% for i in [1,2,3,4,5] %}<input type="radio" name="fear" id="f{{i}}" value="{{i}}" {{ 'checked' if (overview.checkin and overview.checkin.fear==i) or (not overview.checkin and i==3) }}><label for="f{{i}}">{{i}}</label>{% endfor %}</div>
          </div>
          <div class="ck-row"><label>Minutes avail.</label><input type="number" name="minutes" min="0" max="480" value="{{ overview.checkin.available_minutes if overview.checkin else 30 }}"></div>
          <button class="btn green" type="submit">Record check-in</button>
          <div id="ck-msg"></div>
        </form>
      </div>
    </div>
  ...
  <!-- ═══ STREAMS ═══ -->
  <div class="panel p-green reveal" style="margin-top:1rem">
    <header><span class="kicker">The Foundry</span><h2>Revenue streams</h2></header>
    <div class="body">
      <div class="streams" id="streams">
        {% for st in streams %}
        <div class="stream">
          <div class="top">
            <div><h3>{{ st.name }}</h3><div class="slug">{{ st.slug }}</div></div>
            <div style="display:flex;gap:.3rem">
              <span class="badge b-L">L{{ st.autonomy }}</span>
              <span class="badge b-{{ st.status }}">{{ st.status }}</span>
            </div>
          </div>
          <div class="nums">
            <div class="num"><b class="green">${{ '%.2f'|format(st.revenue_30d) }}</b><span>30d rev</span></div>
            <div class="num"><b>${{ '%.2f'|format(st.revenue_total) }}</b><span>all time</span></div>
            <div class="num"><b class="blue">{{ st.effective_hourly }}</b><span>$/hr eff.</span></div>
          </div>
          <div class="mono-sm dim">kill: {{ st.kill_criterion }}</div>
          <div class="killbar"><i style="width:{{ st.kill_progress }}%"></i></div>
          <div class="kill-label">{{ st.kill_label }} · {{ st.ai_calls_30d }} AI calls · margin ~{{ '%.0f'|format(st.margin) }}%</div>
          {{ st.sparkline|safe }}
          <div class="agents-on">
            {% if st.agents %}{% for a in st.agents %}<span class="mono-sm">{{ a.name }}</span> <span class="badge b-{{ a.status if a.status in ('active','paused','killed','pending') else ('active' if a.status in ('idle','running') else 'killed') }}">{{ a.status }}</span> {% endfor %}
            {% else %}<span class="dim">no agents assigned</span>{% endif %}
          </div>
          {% if st.prediction %}
          <div class="mono-sm dim" style="margin-top:.4rem">oracle: predicted {{ st.prediction.predicted_value }} @ {{ st.prediction.predicted_confidence }}% conf · {{ 'actual: '~st.prediction.actual_value if st.prediction.actual_value else 'pending' }}</div>
          {% endif %}
        </div>
        {% endfor %}
      </div>
  ...
  <!-- ═══ AGENT WORKFORCE + ORACLE ═══ -->
  <div class="grid g-2" style="margin-top:1rem">
    <div class="panel p-cyan reveal">
      <header><span class="kicker">Workforce</span><h2>Agents</h2></header>
      <div class="body" style="overflow-x:auto">
        <table>
          <thead><tr><th></th><th>Agent</th><th>Status</th><th>Current task</th><th>Done/Fail</th><th>Err%</th><th>Heartbeat</th><th>Restarts</th></tr></thead>
          <tbody id="agents-body">
          {% for a in agents %}
            <tr class="row">
              <td><span class="hb {{ 'stale' if a.stale else ('ok' if a.status in ('idle','running') else 'off') }}"></span></td>
              <td><b>{{ a.name }}</b><br><span class="tag mono-sm dim">{{ a.agent_type }}{% if a.stream %} · {{ a.stream }}{% endif %}</span></td>
              <td><span class="{{ 'green' if a.status in ('idle','running') else ('amber' if a.status=='paused' else 'red') }}">{{ a.status }}</span></td>
              <td class="dim" style="max-width:180px">{{ (a.current_task or '—')|truncate(60) }}</td>
              <td class="mono-sm">{{ a.tasks_completed or 0 }}/{{ a.tasks_failed or 0 }}</td>
              <td class="mono-sm {{ 'red' if a.error_rate>20 else '' }}">{{ a.error_rate }}%</td>
              <td class="mono-sm dim">{{ a.hb_age }}</td>
              <td class="mono-sm dim">{{ a.restart_count or 0 }}</td>
            </tr>
          {% endfor %}
          </tbody>
        </table>
    ...
    <div class="panel p-purple reveal">
      <header><span class="kicker">The Oracle</span><h2>Prediction & simulation</h2></header>
      <div class="body">
        <div class="mono-sm dim" style="margin-bottom:.4rem">CALIBRATION — accuracy by confidence band</div>
        <div class="cal">
          {% for c in oracle.calibration %}
          <div class="bar"><b>{{ c.accuracy }}%</b><i style="height:{{ c.accuracy }}%" title="{{ c.count }} predictions"></i><span>{{ c.bucket }}</span></div>
          {% else %}
          <div class="dim" style="font-size:.8rem;align-self:center">Not enough measured predictions yet. The Oracle is still learning.</div>
          {% endfor %}
        </div>
        <div class="mono-sm dim" style="margin:1rem 0 .3rem">ACTIVE PREDICTIONS</div>
        {% for p in oracle.active_predictions %}
        <div class="item"><span class="purple mono-sm">{{ p.predicted_value }}</span> @ {{ p.predicted_confidence }}% — {{ p.description|truncate(70) }}<br><span class="tag">{{ p.stream or 'system' }} · {{ p.created_at[:10] }}</span></div>
        {% else %}<div class="dim" style="font-size:.8rem">No active predictions.</div>{% endfor %}
        {% if oracle.simulations %}
        <div class="mono-sm dim" style="margin:1rem 0 .3rem">RECENT SCENARIO SIMULATIONS</div>
        {% for s in oracle.simulations %}
        <div class="item">{{ s.description|truncate(60) }}<br><span class="tag">recommendation: {{ s.sim.get('recommendation','—') }} · key uncertainty: {{ (s.sim.get('key_uncertainty') or '—')|truncate(50) }}</span></div>
        {% endfor %}
        {% endif %}
      </div>
    </div>
  ...
  <!-- ═══ GOVERNANCE + HUMAN ═══ -->
  <div class="grid g-2" style="margin-top:1rem">
    <div class="panel p-amber reveal">
      <header><span class="kicker">Constitutional Court</span><h2>Governance</h2></header>
      <div class="body">
        <div class="mono-sm" style="margin-bottom:.6rem">
          <span class="green">{{ governance.audit_stats.passes }} pass</span> ·
          <span class="red">{{ governance.audit_stats.fails }} blocked</span> ·
          <span class="amber">{{ governance.audit_stats.reviews }} review</span> ·
          <span class="dim">{{ governance.audit_stats.total_audits }} total audits</span>
        </div>
        <div class="mono-sm dim" style="margin-bottom:.3rem">RECENT AUDITS</div>
        {% for au in governance.recent_audits %}
        <div class="item"><span class="{{ 'green' if au.result=='PASS' else ('red' if au.result=='FAIL' else 'amber') }}">{{ au.result }}</span> — {{ au.action_description|truncate(65) }}<br><span class="tag">{{ au.agent }} · {{ au.created_at[:16] }}</span></div>
        {% else %}<div class="dim" style="font-size:.8rem">No audits yet.</div>{% endfor %}
        <div class="mono-sm dim" style="margin:1rem 0 .3rem">RECENT DECISIONS</div>
        {% for d in governance.decisions %}
        <div class="item"><b>{{ d.description|truncate(60) }}</b> → <span class="blue">{{ d.decision|truncate(30) }}</span><br><span class="tag">{{ d.created_at[:16] }}</span></div>
        {% else %}<div class="dim" style="font-size:.8rem">No decisions recorded yet.</div>{% endfor %}
      </div>
    </div>

    <div class="panel p-red reveal">
      <header><span class="kicker">Founder authority</span><h2>Approval queue <span class="mono-sm dim">({{ governance.approvals|length }} pending)</span></h2></header>
      <div class="body" id="approvals">
        {% for ap in governance.approvals %}
        <div class="approval" id="approval-{{ ap.id }}">
          <div class="desc">{{ ap.description }}</div>
          <div class="meta">{{ ap.item_type }}{% if ap.stream %} · {{ ap.stream }}{% endif %} · priority {{ ap.priority }} · {{ ap.created_at[:16] }}</div>
          <div class="btns">
            <button class="btn sm green" onclick="resolve({{ ap.id }},'approved')">Approve</button>
            <button class="btn sm red" onclick="resolve({{ ap.id }},'rejected')">Reject</button>
          </div>
        </div>
        {% else %}
        <div class="dim" style="font-size:.85rem;padding:.5rem 0">Nothing needs your approval. The Institution is running autonomously.</div>
        {% endfor %}
      </div>
    </div>
  </div>
  ...
  <!-- ═══ LESSONS + MILESTONES ═══ -->
  <div class="grid g-2" style="margin-top:1rem">
    <div class="panel p-green reveal">
      <header><span class="kicker">Reflection Database</span><h2>Latest lessons</h2></header>
      <div class="body">
        {% for l in oracle.lessons %}
        <div class="item">{{ l.lesson|truncate(140) }}<br><span class="tag">{{ l.stream or 'general' }} · confidence {{ l.confidence_score }}% · {{ l.created_at[:10] }}</span></div>
        {% else %}<div class="dim" style="font-size:.8rem">No lessons yet. The Institution learns as it acts.</div>{% endfor %}
      </div>
    </div>
    <div class="panel p-cyan reveal">
      <header><span class="kicker">Security Buffer</span><h2>Milestones</h2></header>
      <div class="body">
        {% for m in milestones %}
        <div class="item"><span class="green">◆</span> <b>{{ m.description|replace('MILESTONE: ','') }}</b><br><span class="tag">{{ m.created_at[:10] }} — {{ (m.reasoning or '')|truncate(90) }}</span></div>
        {% else %}<div class="dim" style="font-size:.8rem">No milestones yet. The first revenue will be recorded here — and celebrated.</div>{% endfor %}
      </div>
    </div>
  </div>

  <footer>THE INSTITUTION · founded 2026 · reality always wins · <a href="/manual">stewardship manual</a></footer>
</div>
...
<div class="modal-bg" id="stop-modal">
  <div class="modal">
    ...
    <p>This halts the meta-agent and pauses all agents immediately. Content already published stays published. You can resume at any time. Nothing is lost.</p>
    <div class="btns">
      <button class="btn" onclick="closeStop()">Cancel</button>
      <button class="btn red" onclick="doStop()">Stop the Institution</button>
    </div>
  </div>
</div>

<script>
var refreshSec = 60, remaining = refreshSec;

function fmt(n){ return '$' + (n||0).toFixed(2); }

function tick(){
  remaining--;
  if(remaining <= 0){ remaining = refreshSec; refresh(); }
...
function refresh(){
  fetch('/api/status').then(function(r){ return r.json(); }).then(function(s){
    var o = s.overview;
    document.getElementById('s-runway').textContent = o.runway.runway_days.toFixed(0);
    document.getElementById('s-rev').textContent = fmt(o.revenue_total);
    document.getElementById('s-agents').textContent = o.active_agents + '/' + o.total_agents;
    document.getElementById('s-compliance').textContent = o.compliance.compliance_rate + '%';
    document.getElementById('k-runway').textContent = o.runway.runway_days.toFixed(0) + 'd';
    var tr = o.trend || {};
    var tEl = document.getElementById('k-runway-trend');
    tEl.textContent = tr.trend_label || '—';
    tEl.className = 'sub ' + ((tr.trend_days||0) > 0.5 ? 'trend-up' : ((tr.trend_days||0) < -0.5 ? 'trend-down' : 'trend-flat'));
    document.getElementById('k-rev-all').textContent = fmt(o.revenue_total);
    document.getElementById('k-rev-month').textContent = 'this month: ' + fmt(o.revenue_month);
    document.getElementById('k-agents').textContent = o.active_agents + ' / ' + o.total_agents;
    var svc = o.services || {};
    document.getElementById('k-services').textContent = 'meta:' + (svc['meta-agent']||'?') + ' · safety:' + (svc['safety']||'?');
    var m = o.metrics;
    if(m){
      document.getElementById('k-health').textContent = (m.cpu_percent||0).toFixed(0) + '% cpu';
      document.getElementById('k-health-sub').textContent = 'ram ' + (m.ram_percent||0).toFixed(0) + '% · disk ' + (m.disk_percent||0).toFixed(0) + '%' + (m.gpu_temp ? ' · gpu ' + m.gpu_temp + '°C' : '');
    }
    document.getElementById('k-compliance').textContent = o.compliance.compliance_rate + '%';
    document.getElementById('k-compliance-sub').textContent = o.compliance.fails + ' blocked · ' + o.compliance.reviews + ' reviewed';
    var ck = o.checkin;
    if(ck){
      document.getElementById('k-founder').textContent = 'E' + ck.energy + ' P' + ck.pain + ' F' + ck.fear;
      document.getElementById('k-founder-sub').textContent = ck.available_minutes + ' min available · last check-in ' + (ck.created_at||'').slice(0,16);
    } else {
      document.getElementById('k-founder').textContent = '—';
      document.getElementById('k-founder-sub').textContent = 'no check-in yet';
    }
    var sa = s.single_action || {};
    document.getElementById('action-text').textContent = sa.text || '—';
    var meta = document.getElementById('action-meta');
    var bits = [];
    if(sa.minutes) bits.push('<b>' + sa.minutes + ' min</b>');
    if(sa.expected_runway) bits.push('<b>+' + sa.expected_runway + ' days</b> runway');
    if(sa.risk) bits.push('risk: <b>' + sa.risk + '</b>');
    meta.innerHTML = bits.join(' · ');
  }).catch(function(e){ console.error('refresh failed', e); });
}

function sendCheckin(ev){
  ev.preventDefault();
  var f = ev.target;
  var payload = {
    energy: f.energy.value, pain: f.pain.value, fear: f.fear.value,
  ...
  fetch('/api/checkin', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload)})
    ...
    .then(function(d){
      document.getElementById('ck-msg').textContent = d.message || 'Recorded.';
      refresh();
    });
  return false;
}
...
function resolve(id, status){
  fetch('/api/approval/' + id + '/resolve', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({status: status})})
    .then(function(r){ return r.json(); })
    .then(function(d){
      var el = document.getElementById('approval-' + id);
      if(el){ el.style.opacity = '0.35'; el.innerHTML = '<div class="desc">' + (status==='approved' ? '✓ Approved' : '✗ Rejected') + '</div>'; }
    });
}

function openStop(){ document.getElementById('stop-modal').classList.add('show'); }
function closeStop(){ document.getElementById('stop-modal').classList.remove('show'); }
function doStop(){
  fetch('/api/emergency-stop', {method:'POST'}).then(function(){ location.reload(); });
}
function resume(){
  fetch('/api/resume', {method:'POST'}).then(function(){ location.reload(); });
}
...
if __name__ == "__main__":
    ...
    port = int(os.environ.get("DASHBOARD_PORT", config.get("dashboard", "port", default=8080)))
    logger.info(f"Dashboard starting on {host}:{port} (auth {'enabled' if AUTH_ENABLED else 'disabled'})")
