...
class MetaAgent:
            ...
            try:
                ...
                if stream:
                    ...
                    if stream_cfg and stream_cfg["status"] != "active":
                        logger.info(f"Skipping {agent_name}: stream '{stream}' is {stream_cfg['status']}")
                        continue
    ...
    def _dispatch_task(self, task: dict):
        ...
        try:
            ...
            self.db.complete_task(task["id"], str(result)[:500] if result else "Completed")
    ...
    def _evaluate_kill_criterion(self, stream: dict) -> bool:
        ...
        threshold = stream.get("kill_threshold", 0)
    ...
    def _kill_stream(self, stream: dict):
        ...
        audit = self.court.audit(
            action=f"Kill revenue stream '{slug}' due to: {stream['kill_criterion']}",
            agent="meta_agent",
            stream=slug,
        )
    ...
    def _measure_prediction_outcome(self, prediction: dict) -> Optional[float]:
        ...
        if "revenue" in desc_lower and stream:
    ...
    def _generate_digest(self):
        ...
        digest = self._format_digest(
            ...
            completed=completed_today["cnt"] if completed_today else 0,
    ...
    def _format_digest(self, **kwargs) -> str:
        ...
        if energy <= 1:
            ...
            greeting = "Steady progress. Here's what matters today."
        else:
            greeting = "Good energy. Here's your strategic overview."

        # Fear-adaptive section
        fear_section = ""
        if fear <= 2:
            fear_section = f"""
## 🛡 SECURITY BUFFER — FEAR REDUCTION
Your runway is **{runway:.0f} days**. The system is actively working to increase this.
Every day, agents scan for grants, opportunities, and efficiency gains.
You are not alone in this. The Institution is building your safety net.
"""

        # Stream summary
        stream_lines = []
        for s in streams[:5]:
            stream_lines.append(f"  - **{s['name']}**: {s['status']} (L{s.get('autonomy_level', 1)})")
        stream_summary = "\n".join(stream_lines) if stream_lines else "  - No active streams yet."

        # Agent summary
        active_agents = len([a for a in agents if a.get("status") in ("idle", "running")])
        total_agents = len(agents)

        # Lessons
        lesson_lines = []
        for l in lessons[:3]:
            lesson_lines.append(f"  - {l.get('lesson', 'N/A')[:100]}")
        lesson_summary = "\n".join(lesson_lines) if lesson_lines else "  - No recent lessons."

        # System health
        if metrics:
            health_line = f"CPU: {metrics.get('cpu_percent', 'N/A')}% | RAM: {metrics.get('ram_percent', 'N/A')}% | Disk: {metrics.get('disk_percent', 'N/A')}%"
        else:
            health_line = "Metrics not yet collected."

        digest = f"""# THE INSTITUTION — DAILY DIGEST
...
| Tasks Completed (24h) | {completed} |
| Tasks Failed (24h) | {failed} |
| Pending Approvals | {len(pending)} |
| Constitutional Compliance | {audit_stats.get('compliance_rate', 100)}% |
| System Health | {health_line} |
...
- Total audits: {audit_stats.get('total_audits', 0)}
- Passes: {audit_stats.get('passes', 0)}
- Blocked: {audit_stats.get('fails', 0)}
- Reviews: {audit_stats.get('reviews', 0)}

---

*Generated automatically by The Institution. Next digest: tomorrow 07:00.*
*Dashboard: http://localhost:{self.config.get('dashboard', 'port', default=8080)}*
"""
        return digest

    def generate_weekly_report(self):
        """Public method for cron-triggered weekly report."""
        self._generate_weekly_report()
    ...
    def _generate_weekly_report(self):
        ...
        revenue_week = self.db.fetchone(
            "SELECT COALESCE(SUM(amount), 0) as total FROM revenue WHERE recorded_at > ?",
            (week_ago,)
        )
        tasks_week = self.db.fetchone(
            "SELECT COUNT(*) as cnt FROM tasks WHERE created_at > ?", (week_ago,)
        )
        completed_week = self.db.fetchone(
            "SELECT COUNT(*) as cnt FROM tasks WHERE status = 'completed' AND completed_at > ?",
            (week_ago,)
        )
        lessons_week = self.db.get_lessons(limit=10)
        streams = self.db.get_active_streams()

        # AI usage stats
        ai_usage = self.db.fetchone(
            "SELECT COUNT(*) as calls, COALESCE(SUM(total_tokens), 0) as tokens FROM ai_usage WHERE created_at > ?",
            (week_ago,)
        )

        report = f"""# THE INSTITUTION — WEEKLY STRATEGIC REPORT
**Week of {datetime.now().strftime('%B %d, %Y')}**

## Executive Summary

- Revenue this week: ${revenue_week['total'] if revenue_week else 0:.2f}
- Tasks completed: {completed_week['cnt'] if completed_week else 0}/{tasks_week['cnt'] if tasks_week else 0}
- Active streams: {len(streams)}
- AI calls: {ai_usage['calls'] if ai_usage else 0} ({ai_usage['tokens'] if ai_usage else 0} tokens)
- Lessons learned: {len(lessons_week)}

## Stream Performance

| Stream | Status | Autonomy | Revenue (30d) |
|--------|--------|----------|---------------|
"""
        for s in streams:
            ...
            report += f"| {s['name']} | {s['status']} | L{s.get('autonomy_level', 1)} | ${rev['total'] if rev else 0:.2f} |\n"

        report += f"""
## Lessons This Week

"""
        for l in lessons_week[:5]:
            report += f"- **{l.get('stream', 'general')}**: {l.get('lesson', 'N/A')[:150]}\n"
...
"""
        ...
        if not streams:
            report += "- No active streams. Priority: Activate first revenue stream.\n"
        else:
            # Find worst performer
            report += "- Review underperforming streams against kill criteria.\n"
            report += "- Consider spawning new agents for validated opportunities.\n"

        report += f"""
## Next Week Focus

1. Continue autonomous operations
2. Monitor stream performance trends
3. Process founder approvals promptly
4. Maintain prediction calibration

---
*Generated by Meta-Agent. Institutional memory updated.*
"""

        # Save report
        reports_dir = INSTITUTION_ROOT / "reports" / "weekly"
        reports_dir.mkdir(parents=True, exist_ok=True)
        report_path = reports_dir / f"weekly_{today_str()}.md"
        report_path.write_text(report, encoding="utf-8")
        logger.info(f"Weekly report saved to {report_path}")

    def _strategic_review(self):
        """Weekly strategic review using Oracle and Reflection Database."""
        logger.info("Running strategic review...")

        # Query lessons for strategic insights
        lessons = self.db.get_lessons(limit=20)
        streams = self.db.get_active_streams()

        if not lessons and not streams:
            return

        # Use AI to synthesize strategic insights
        prompt = f"""You are the strategic advisor for The Institution.

ACTIVE STREAMS: {json.dumps([s['name'] for s in streams], default=str)}

RECENT LESSONS:
{json.dumps([l.get('lesson', '') for l in lessons[:10]], default=str)}

Based on this data, provide:
1. One strategic insight (what pattern do you see?)
2. One risk to watch (what could go wrong?)
3. One opportunity to pursue (what should we try next?)

Be specific and actionable. Maximum 3 sentences each.
"""
        try:
            ...
            if response:
                self.db.insert("decisions", {
                    ...
                    "reasoning": "AI-synthesized strategic insights from lessons and stream data",
                    "agent": "meta_agent",
                })
        except Exception as e:
            logger.warning(f"Strategic review AI synthesis failed: {e}")

    # ═══════════════════════════════════════════════════════════
    # AGENT SPAWNING (META-AGENT RECURSIVE IMPROVEMENT)
    # ═══════════════════════════════════════════════════════════

    def spawn_agent_for_opportunity(self, opportunity: dict) -> bool:
        """
        Spawn a new agent when the Niche Scout identifies a validated opportunity.
        ...
        if score < min_score:
            logger.info(f"Opportunity score {score} below threshold {min_score}. Not spawning.")
            return False

        # Constitutional audit
        audit = self.court.audit(
            action=f"Spawn new agent for opportunity: {opportunity.get('description', 'unknown')}",
            agent="meta_agent",
            stream=opportunity.get("stream"),
        )

        if audit.overall_result == "FAIL":
            logger.warning(f"Constitutional Court blocked agent spawn: {audit.reasoning}")
            return False

        if audit.requires_approval:
            self.court.create_approval_request(audit)
            logger.info("Agent spawn requires founder approval. Queued.")
            return False

        # Check max concurrent streams
        active = self.db.get_active_streams()
        max_streams = self.config.get("meta_agent", "spawn_threshold", "max_concurrent_streams", default=15)
        if len(active) >= max_streams:
            logger.warning(f"Max concurrent streams ({max_streams}) reached. Cannot spawn.")
            return False

        # Register the new stream
        slug = opportunity.get("slug", f"stream_{int(time.time())}")
        self.db.insert("streams", {
            "name": opportunity.get("name", f"New Stream {slug}"),
            "slug": slug,
            "status": "active",
            "autonomy_level": 1,
            "kill_criterion": opportunity.get("kill_criterion", "No revenue after 60 days"),
            "kill_threshold": 0,
            "kill_metric": "revenue_total",
            "kill_window_days": 60,
        })

        logger.info(f"New stream spawned: {slug}")
        return True
    ...
    def _shutdown(self):
        """Graceful shutdown: save state, stop agents, log."""
        logger.info("Meta-agent shutting down gracefully...")

        # Mark all running agents as idle
        self.db.execute("UPDATE agents SET status = 'idle', current_task = NULL WHERE status = 'running'")

        # Log shutdown
        self.db.log_incident("info", "meta_agent", "Meta-agent shut down gracefully.")

        logger.info("═══ THE INSTITUTION META-AGENT STOPPED ═══")
