...
class RateLimiter:
    ...
    def record_429(self, provider: str):
        with self._lock:
            ...
            self.db.execute(
                "UPDATE rate_limits SET consecutive_429s = ?, backoff_until = ? WHERE provider = ?",
...
class AIProviderManager:
    ...
    # ─── OPENROUTER ───────────────────────────────────────────
    def _call_openrouter(self, prompt: str, model: str, max_tokens: int, temperature: float) -> Optional[str]:
    ...
    # ─── OLLAMA (LOCAL) ───────────────────────────────────────
    def _call_ollama(self, prompt: str, model: str, max_tokens: int, temperature: float) -> Optional[str]:
...
"""
    ...
    def _template_product(self, prompt: str) -> str:
...
3. Tertiary objective with evaluation method
...
The system will retry with live providers on the next cycle.
