...
class NicheScoutAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        self.score_weights = {
            ...
            "reach": 0.10,         # Can we reach the buyer cheaply?
    ...
    def _discover_from_performance(self) -> list:
        ...
        revenue_by_stream = self.db.fetchall(
            """SELECT stream, SUM(amount) as total, COUNT(*) as count
               FROM revenue GROUP BY stream ORDER BY total DESC"""
        )
    ...
    def _discover_via_ai(self) -> list:
        ...
        stream_names = [s["name"] for s in active_streams] if active_streams else []
...
"""
                    response = self.generate_text(prompt, quality_tier="routine", temperature=0.8, max_tokens=800)
                    if response:
                        try:
                            json_start = response.find("[")
                            json_end = response.rfind("]") + 1
            ...
            for i in range(min(3, len(niches))):
                for j in range(i + 1, min(4, len(niches))):
                    ...
                    opportunities.append({
                        ...
                        "target_buyer": f"People interested in both {niche_a} and {niche_b}",
...
"""
    ...
    def _score_opportunities(self, opportunities: list) -> list:
        ...
        for opp in opportunities:
            ...
            # Ability: Can we deliver with AI?
            ...
            compounding_keywords = ["template", "system", "tool", "platform", "library", "framework"]
        ...
        for approval in approved:
            ...
            try:
                ...
                success = meta.spawn_agent_for_opportunity({
                    ...
                    "stream": slugify(opp.get("title", f"stream_{opp_hash}")),
                })
