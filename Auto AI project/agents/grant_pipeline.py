...
class GrantPipelineAgent(BaseAgent):
    ...
    def run_once(self):
        """Main cycle: discover grants, score, draft applications, track status."""
    ...
    def _parse_business_gov(self, soup: BeautifulSoup, base_url: str) -> list:
        ...
        grant_cards = soup.select(".card, .grant-item, .search-result, article, .list-item")
        if not grant_cards:
    ...
    def _filter_eligible(self, grants: list, eligibility: dict) -> list:
        """Filter grants by eligibility profile."""
        if not grants:
            return []
        ...
        for grant in grants:
            ...
            energy_keywords = ["energy", "solar", "efficiency", "renewable", "sustainability"]
    ...
    def _score_grants(self, grants: list, weights: dict) -> list:
        ...
        for grant in grants:
            ...
            if grant_type == "energy":
                ...
                effort_score = min(10, effort_score + 1)
