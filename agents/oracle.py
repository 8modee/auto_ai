  ...
  if error_pct > 30:
    ...
    def _measure_single_outcome(self, prediction: dict) -> Optional[float]:
        ...
        if "task" in desc_lower and ("complet" in desc_lower or "finish" in desc_lower):
            row = self.db.fetchone(
                "SELECT COUNT(*) as cnt FROM tasks WHERE status = 'completed' AND completed_a
t > datetime('now', '-7 days')"
            )
            ...
            if current_7d > 0:
                ...
                return {
                    ...
                    "scenario_data": {
                        ...
                        "key_assumption": "Current 7-day trend 
continues",
    ...
    def _run_simulations(self) -> int:
        ...
        pend
ing_decisions = self.db.fetchall(
...
- Risk level (1-10)
- Prob
ability of this scenario occurring
