...
class SecurityBufferAgent(BaseAgent):
    ...
    def run_once(self):
        """Main cycle: snapshot run
way, check milestones, reduce fear, rank actions."""
        results = []
...
), 0) as total FROM expenses WHERE recorded_at > datetime('now', '-30 days')"
        )
    ...
    def _calculate_trend(self, history: list, current: dict) -> dict:
        ...
        return {
            ...
            "trend_label": f"{direction} {delta:+.1
f} days vs last week",
    ...
    def _celebrate_milestone(self, milestone: dict, snapshot: dict):
        ...
        self.db.insert("decisions", {
            ...
            "reasoning": (
                ...
                f"(balance ${snapshot['balance']:.2f}, net {snapsh
ot['net_daily']:+.2f}/day). "
...
actua
l ledger, not optimism. If the numbers were bad, it would say so.*
"""
...
get("budget") or 150
    ...
    def security_buffer_section(self) -> str:
        ...
        rev_count = week_revenue["cnt"
] if week_revenue else 0
